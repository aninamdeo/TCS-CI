...
class News_Desp extends CI_Controller
{
 ...
 public function index($per_page = 30)
 {
    ...
    $paginate = $this->basic->create_links('Backend/News_Desp/index/', $total, $per_page,6);
    ...
    $this->load->view('auth/backend/admin/News_Desp/News_Desp',$data);
...
public function add_news_desp()
{
    ...
    $this->load->view('auth/backend/admin/News_Desp/Add_News_Desp',$data);
    $this->basic->footer($mid);
}
...
{  
   ...
   $this->load->view('auth/backend/admin/News_Desp/Edit_News_Desp',$data);
   $this->basic->footer($mid);
}
...
public function comments($id,$per_page=20)
{
    ...
    $paginate = $this->basic->create_links('Backend/News_Desp/comments/'.$id.'/', $total, $per_page,7);
    $data['links'] = $paginate['links'];
    ...
    $this->load->view('auth/backend/admin/News_Desp/all_news_comments',$data);
    $this->basic->footer($mid);
}
...
{  
   ...
   $this->load->view('auth/backend/admin/News_Desp/edit_comment',$data);
   $this->basic->footer($mid);
}
public function edit_subcomment($ce_id) {
    $data['comment'] = $total = $this->Query->select('*', 'news_comments', ['id' => $ce_id], 'row');
    $this->load->view('auth/backend/admin/News_Desp/edit_subcomment',$data);
}
