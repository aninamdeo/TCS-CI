...
public function index($per_page = 30)
    {
        ...
        $paginate = $this->basic->create_links('Backend/Video_Bulletin/video/', $total, $per_page, 6);
        ...
        $this->load->view('auth/backend/admin/Video_Bulletin/view_bulletin',$data);
    ...
    public function add_bulletin()
  {
        ...
        $this->basic->header($mid, 'Front');
        $this->load->view('auth/backend/admin/Video_Bulletin/add_bulletin'); 
        $this->basic->footer($mid);
    }
  public function search($per_page = 30) {
        ...
        $paginate = $this->basic->create_links('Backend/Video_Bulletin/index/', $total, $per_page,6);
        $data['links'] = $paginate['links'];
        $data['Video_Bulletin_data'] = $total = $this->Query->select('*', 'video_bulletin', $search, 'result', '', [$paginate['per_page'], $paginate['page']]);
        $this->load->view('auth/backend/admin/Video_Bulletin/video_bulletin',$data);
        $this->basic->footer($mid);
    }
    public function insert_bulletin()
    {
       ...
       $qry= $this->Query->insert('video_bulletin',$array);
        ...
        redirect('backend/Video_Bulletin/video');
    }
    ...
    {  
        ...
        $this->load->view('auth/backend/admin/Video_Bulletin/edit_bulletin',$data);
    }
    public function update_bulletin($id)
    {
     ...
     redirect('backend/Video_Bulletin');
    }
