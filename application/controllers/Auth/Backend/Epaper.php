...
class Epaper extends CI_Controller
{
 ...
 public function index($per_page = 30)
 {
    ...
    $total = $this->Query->select('*', 'epaper', '', 'count');
    $paginate = $this->basic->create_links('Backend/Epaper/index/', $total, $per_page,6);
    $data['links'] = $paginate['links'];
    $data['Epaper'] = $total = $this->Query->select('*','epaper', '', 'result',['id'=>'desc'],[$paginate['per_page'], $paginate['page']]);
    $this->load->view('auth/backend/admin/Epaper/Epaper',$data);
...
public function search($per_page = 30) {
    ...
    $total = $this->Query->select('*', 'epaper', $search, 'count');
    $paginate = $this->basic->create_links('Backend/Epaper/index/', $total, $per_page,6);
    $data['links'] = $paginate['links'];
    $data['Epaper'] = $total = $this->Query->select('*', 'epaper', $search, 'result',['id'=>'desc'],[$paginate['per_page'], $paginate['page']]);
    $this->load->view('auth/backend/admin/Epaper/Epaper',$data);
    $this->basic->footer($mid);
}
public function add_Epaper()
{
    $mid=mid;
    $data['permit'] = chk($mid, ['add', 'view','edit','delete']);
    $this->basic->header($mid, 'Front');
    ...
    $this->load->view('auth/backend/admin/Epaper/Add_Epaper',$data);
    $this->basic->footer($mid);
}
public function insert_Epaper($new_add='')
{
    $image ='';
    $vtype = '';
    if($_FILES['image']['name']!=""){
        $image=$this->basic->file_upload('Epaper_Images/news_image','image');
    }else{
        $image = '';
    } 
    ...
    $video=$this->basic->file_upload('Epaper_Images/news_video','video');            
}else{
    $video = '';
}
...
$last_id=$this->Query->insert('epaper',$array,true);
if($last_id> 0) {
...
public function edit_Epaper($id)
{  
   $mid=65;
   $data['permit'] = chk($mid, ['edit']);
   $this->basic->header($mid, 'Front');
   $data['Epaper'] = $news=$this->Query->select('*','epaper',['id'=>$id],'row');
   $data['category']=$this->Query->select('*','category',['type'=>'Category','status'=>'Enabled']);
   ...
   $this->load->view('auth/backend/admin/Epaper/Edit_Epaper',$data);
   $this->basic->footer($mid);
}
public function update_Epaper($id)
{
    $Epaper=$this->Query->select('*','epaper',['id'=>$id]);
    $vtype = '';
    if($_FILES['image']['name']!="")
    {
        $image=$this->basic->file_upload('Epaper_Images/news_image','image');
    ...
    $Epaper_data = [  'cat_id' => $this->input->post('category'),                   
    'sub_cat_id'=>$this->input->post('subcategory'),
    'area_hindi'=>$this->input->post('area_hindi'),
    'title_hindi'=>$this->input->post('title_hindi'),
    'content_hindi'=>$this->input->post('content_hindi'),
...
if($this->Query->update('epaper',['id'=>$id],$Epaper_data))
{
...
public function delete_Epaper($id)
{
    $mid=65;
    $data['permit'] = chk($mid, ['delete']);
    $Epaper=$this->Query->select('*','epaper',['id'=>$id]);
    $qry= $this->Query->delete('epaper',['id'=>$id]);
...
public function comments($id,$per_page=20)
{
    ...
    $paginate = $this->basic->create_links('Backend/Epaper/comments/'.$id.'/', $total, $per_page,7);
    $data['links'] = $paginate['links'];
    $data['values']=$this->Query->select('*','news_comments',['news_id'=>$id,'type'=>'Main_Comment'],'',['id'=>'desc'],[$paginate['per_page'], $paginate['page']]);
    $news = $this->Query->select('*','epaper',['id'=>$id],'row');
    $data['cat_id']=  $news->cat_id;
    $this->load->view('auth/backend/admin/Epaper/all_news_comments',$data);
    $this->basic->footer($mid);
}
...
{  
   ...
   $this->load->view('auth/backend/admin/Epaper/edit_comment',$data);
   $this->basic->footer($mid);
}
public function edit_subcomment($ce_id) {
    $data['comment'] = $total = $this->Query->select('*', 'news_comments', ['id' => $ce_id], 'row');
    $this->load->view('auth/backend/admin/Epaper/edit_subcomment',$data);
}
...
public function curlsender($uid,$id){
    $n_query = $this->db->select("category.*,sub_category.*,epaper.*")->from('epaper')->join('category','epaper.cat_id=category.id','left')->join('sub_category','epaper.sub_cat_id=sub_category.sub_category_id','left')->where('epaper.id',$id)->get();
        $news = $n_query->row();
