...
class Basic_Details extends CI_Controller {
    ...
    public function index($per_page = 30) {
        ...
        $data['values'] = $this->Query->select('*', 'basic_details', ['id' =>1], 'row');
        $this->load->view('auth/website/basic_details/basic_view', $data);
        $this->basic->footer($mid);
    }

    public function edit() {
        $data['values'] = $total = $this->Query->select('*', 'basic_details', ['id' =>1], 'row');
    ...
    public function update() {
        ...
        $qry = $this->Query->update('basic_details',['id' => 1], $array);
        if ($qry == TRUE) {
