...
class Seven_inews extends CI_Controller {
...
protected function mailerFunction($msg, $to, $sub, $link = '',$name='')
   ...
   public function single_news($id='',$title='') {
        ...
        $url =  'Seven_inews/single_news/'.$id.'/'.preg_replace('/-+/','_',preg_replace('/[^A-Za-z0-9\-]/','_',$news->title));
    ...
    public function single_video($id='',$title='') {
         ...
         //                  'url' => 'Seven_inews/single_video/'.$id.'/'.$video->title_share,
         //                  'image' => $video->image,
         ...
         //                  'video' => $video->video    
         //                );
        $url =  'Seven_inews/single_video/'.$id.'/'.preg_replace('/-+/','_',preg_replace('/[^A-Za-z0-9\-]/','_',$video->title_share));
    ...
    public function single_photo($id='',$title='') {
        ...
        //                  'url' => 'Seven_inews/single_photo/'.$id.'/'.$photo->title_share,
        //                  'image' => $photo->image,
        ...
        //               );
        // $this->header('',$share);
        $url =  'Seven_inews/single_photo/'.$id.'/'.preg_replace('/-+/','_',preg_replace('/[^A-Za-z0-9\-]/','_',$photo->title_share));
     ...
     public function single_gallery($id='',$title='') {
        ...
        //  $share = array( 'title' => $video->title_hindi,
        //                  'url' => 'Seven_inews/single_gallery/'.$id.'/'.$video->title_share,
        //                  'image' => $video->image,
        //                   'meta_title' => $video->title_share,  
        //                   'meta_key' => $video->meta_key,  
        //                   'meta_content' => $video->meta_content,  
        ...
        //                 );
        // $this->header('',$share);
        $url =  'Seven_inews/single_gallery/'.$id.'/'.preg_replace('/-+/','_',preg_replace('/[^A-Za-z0-9\-]/','_',$video->title_share));
        $this->header('',$video->title_hindi,$video->title_share,$video->meta_key,$video->meta_content,$video->image,$url);
    ...
    if(count((array)$user_chk) > 0) {
        if($user_chk->status == 'Enabled')
        {
            ...
            if($user_chk->verification_status == 'Verified'){
              ...
              redirect(base_url('Seven_inews/mobile_verification'));
            }
        }else{
      ...
      if($last_id > 0){
        ...
        redirect(base_url('Seven_inews/login'));
    }
   } 
 }
  public function mobile_verification() {
    ...
    if(count((array)$check_user) > 0) {
      ...
      $qry = $this->Query->update('user',['id'=>$this->session->userdata('user_id')], $array);
        ...
        redirect(base_url('Seven_inews/profile'));
    } else {
         $this->session->set_flashdata('swal_model', 'true');
        // $this->session->set_flashdata('msg','verified');
