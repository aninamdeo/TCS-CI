...
class Category extends CI_Controller
{
    ...
    public function index($per_page = 30)
    {
        ...
        $paginate = $this->basic->create_links('Seven_inews/Category/index/', $total, $per_page,6);
        $data['links'] = $paginate['links'];
        $data['category']=$this->Query->select('*','category','','',['id'=>'desc'],[$paginate['per_page'], $paginate['page']]);
        $this->load->view('auth/seven_inews/admin/Category/Category',$data);
    ...
    public function add_category()
    {
        ...
        $this->load->view('auth/seven_inews/admin/Category/Add_Category');
        $this->basic->footer($mid);
    }
    public function insert_category()
    {
      ...
      $qry =  $this->Query->insert('category',$array);
        ...
        redirect('Auth/Seven_inews/Category/add_category');
    }
    public function edit_category($id)
    {  
        $data['category']=$this->Query->select('*','category',['id'=>$id],'row');
        $this->load->view('auth/seven_inews/admin/Category/Edit_Category',$data);
    }
        ...
        if ($this->Query->update('category',['id'=>$id],$category_data))
            {
               ...
               redirect('Auth/Seven_inews/Category/');
            }else{
                    set_msg('Error Occured. Try Again!!', 'E');
                    redirect('Auth/Seven_inews/Category/');
            } 
    ...
    public function subcategory($cid,$per_page = 30)
    {
         ...
         $paginate = $this->basic->create_links('Seven_inews/Category/subcategory/'.$cid.'/', $total, $per_page,7);
         $data['links'] = $paginate['links'];
         $data['sub_category']=$this->Query->select('*','sub_category',['cat_id'=>$cid],'',['sub_ordering'=>'asc'],[$paginate['per_page'], $paginate['page']]);
        $this->load->view('auth/seven_inews/admin/Category/category_wise_subcategory',$data);
        $this->basic->footer($mid);
     ...
     public function news($id,$per_page = 30)
    {
        ...
        if($id == '1'){
            ...
            $paginate = $this->basic->create_links('Seven_inews/News_Desp/news/'.$id.'/', $total, $per_page,7);
            $data['links'] = $paginate['links'];
        ...
        $this->load->view('auth/seven_inews/admin/Category/category_wise_news',$data);
        $this->basic->footer($mid);
    }
    ...
    public function add_news($id)
    {
      ...
      $data['sub_cat']=$this->Query->select('*','sub_category',['status'=>'Enabled','cat_id'=>$id]);
        ...
        $this->load->view('auth/seven_inews/admin/Category/Add_News',$data);
        $this->basic->footer($mid);
    }
}
?>