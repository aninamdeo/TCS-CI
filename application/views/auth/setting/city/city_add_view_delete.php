    ...
    <div class="content-body">
        <section id="descriptioin" class="card">
            <div class="card-header">
                <h4 class="card-title"><i class="ft-user"></i> Add City</h4>
            </div>
                        ...
                        <div class="form-body">
                            <div class="form-group row">
                                <div class="col-md-3">
                                    ...
                                    <select name="country_id" class="form-control" required onchange='get_state(this.value)'>
                                ...
                                <div class="col-md-3">
                                    <button type="submit" class="btn btn-primary cus-btn" formaction="<?= base_url('Auth/Setting/City/insert') ?>">
                                        Save
                                    </button>
        ...
        <section id="descriptioin" class="card">
            <div class="card-header">
                <h4 class="card-title"><i class="ft-user"></i> View City</h4>
            </div>
            <div class="card-content">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            foreach ($values as $value) {
                                ...
                                <tr>
                                    <td><?= $n ?></td>
                                    <td><?= $value->common_ad_city_name ?></td>
                                    <td><span class="badge <?php echo $value->common_ad_city_status == 'Enabled' ? 'badge-primary' : 'badge-danger'; ?>"><?= $value->common_ad_city_status ?></span></td>
                                    <td><?= $value->common_ad_city_ordering ?></td>
                                    <td>
                                        <a class="btn btn-info cus-btn2" href="#" onclick="edit('<?= $value->common_ad_city_id ?>')">
                                            <i class="la la-pencil"></i>
                                        </a>
                                        <a class="btn btn-danger cus-btn2" onclick="delete_data('<?= $value->common_ad_city_id ?>')" href="#">
...
<script>
    function get_state(value){
        $.get("<?= base_url('Auth/Common/Common/get_state') ?>/" + value, function (data) {
            $('#state').html(data);
        });
    }
    ...
    function edit(value) {
        $.get("<?= base_url('Auth/Setting/City/edit') ?>/" + value, function (data) {
    ...
    function delete_data(value) {
        swal({
            ...
            dangerMode: true,
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({url: "<?= base_url('Auth/Setting/City/delete') ?>/" + value, success: function (result) {
